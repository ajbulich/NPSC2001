...
h=0.005;
tauList=[0.1e-4 0.25e-4 0.5e-4 0.95e-4 1e-4  2e-4];
x=h:h:1;
m=length(x);
...
for tau=tauList
    ...
    k=75;        % average wavenumber (average wavelength=2*pi/k)
    sigma=0.075;    % width-parameter for the Gaussian
    ...
    pos(t) = x(max(prob)==prob);
    t = t + 1;
    
    %figure(2)
    %plot(0:tau:tau*(n-1), Ptot, 'bo-')
end
