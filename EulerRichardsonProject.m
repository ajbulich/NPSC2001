...
for i=1:steps
  ...
  obj1 = update(x,y,z,xvel,yvel,zvel,mArray, 1, h);
  obj2 = update(x,y,z,xvel,yvel,zvel,mArray, 2, h);
  obj3 = update(x,y,z,xvel,yvel,zvel,mArray, 3, h);
  obj4 = update(x,y,z,xvel,yvel,zvel,mArray, 4, h);
  obj5 = update(x,y,z,xvel,yvel,zvel,mArray, 5, h);
  obj6 = update(x,y,z,xvel,yvel,zvel,mArray, 6, h);
  obj7 = update(x,y,z,xvel,yvel,zvel,mArray, 7, h);
 
    
  x(1) = x(1)+h*obj1(1);
  y(1) = y(1)+h*obj1(2);
  z(1) = z(1)+h*obj1(3);
  xvel(1) = xvel(1)+h*obj1(4);
  yvel(1) = yvel(1)+h*obj1(5);
  zvel(1) = zvel(1)+h*obj1(6);

  x(2) = x(2)+h*obj2(1);
  y(2) = y(2)+h*obj2(2);
  z(2) = z(2)+h*obj2(3);
  xvel(2) = xvel(2)+h*obj2(4);
  yvel(2) = yvel(2)+h*obj2(5);
  zvel(2) = zvel(2)+h*obj2(6);

  x(3) = x(3)+h*obj3(1);
  y(3) = y(3)+h*obj3(2);
  z(3) = z(3)+h*obj3(3);
  xvel(3) = xvel(3)+h*obj3(4);
  yvel(3) = yvel(3)+h*obj3(5);
  zvel(3) = zvel(3)+h*obj3(6);

  x(4) = x(4)+h*obj4(1);
  y(4) = y(4)+h*obj4(2);
  z(4) = z(4)+h*obj4(3);
  xvel(4) = xvel(4)+h*obj4(4);
  yvel(4) = yvel(4)+h*obj4(5);
  zvel(4) = zvel(4)+h*obj4(6);

  x(5) = x(5)+h*obj5(1);
  y(5) = y(5)+h*obj5(2);
  z(5) = z(5)+h*obj5(3);
  xvel(5) = xvel(5)+h*obj5(4);
  yvel(5) = yvel(5)+h*obj5(5);
  zvel(5) = zvel(5)+h*obj5(6);

  x(6) = x(6)+h*obj6(1);
  y(6) = y(6)+h*obj6(2);
  z(6) = z(6)+h*obj6(3);
  xvel(6) = xvel(6)+h*obj6(4);
  yvel(6) = yvel(6)+h*obj6(5);
  zvel(6) = zvel(6)+h*obj6(6);

  x(7) = x(7)+h*obj7(1);
  y(7) = y(7)+h*obj7(2);
  z(7) = z(7)+h*obj7(3);
  xvel(7) = xvel(7)+h*obj7(4);
  yvel(7) = yvel(7)+h*obj7(5);
  zvel(7) = zvel(7)+h*obj7(6);
...
function value = update(x,y,z,xvel, yvel, zvel, mArray, massNo, h)
...
for j = 1:length(mArray)
    if j ~= massNo
        ...
        r = norm(pos);
        v = norm(vel);
        acc = acc + (-1*mArray(j) + k*v^2)/(r^2) * pos/r;
    end
end
vmid = vel + 0.5*acc*h;
xmid = Cartesian_pos + 0.5*vel*h;
for j = 1:length(mArray)
    if j ~= massNo
        Cartesian_pos2 = [x(j) y(j) z(j)];
        pos = xmid - Cartesian_pos2;
        r=norm(pos);
        v = norm(vmid);
        amid = amid + (-1*mArray(j) + k*v^2)/(r^2) * pos/r;
    end
end
value(1:3)=vmid;
value(4:6)=amid;
