...
for ii=1:NumExp
    ...
    XSpec = fft(XSig);  %Calculate the spectrum of the signal, this is X
    X = XSpec;
    XStar = conj(X);
    ...
    noise = randn(size(XSig));
    
    YSig = ifft(YSpec);  %Finally inverse fft to get the fitered signal
    Z = YSig + noiseStdDev * noise;

    ZXStar = ZXStar + (fft(Z) .* XStar);
    XXStar = XXStar + (X .* XStar);
end
ZXStarAvg = 1/NumExp * ZXStar;
XXStarAvg = 1/NumExp * XXStar;

H = ZXStarAvg./XXStarAvg;
magH = sqrt(H .* conj(H));
plotAv = abs(AvAll(1:length(AvAll)/2+1));
figure(5)
subplot(2,1,1)
hold on
plot(FVec1, magH(1:length(magH)/2+1), 'r-')
plot(FVec1, plotAv, 'b-')
legend('magH', 'Av')
ylabel('Calculated Spectrum')
ylim([0 1])
hold off
subplot(2,1,2)
hold on
plot(FVec1, rad2deg(angle(H(1:length(magH)/2+1))), 'r-');
plot(FVec1, rad2deg(angle(AvAll(1:length(AvAll)/2+1))), 'b-');
ylabel('Phase (deg) ');
xlabel('Frequency (Hz)');
hold off

figure(6)
plot(FVec1, ZXStarAvg(1:length(ZXStarAvg)/2+1));
title('ZX*')
figure(7)
plot(FVec1, XXStarAvg(1:length(XXStarAvg)/2+1));
title('XX*')
