function NonLinearLSQExample2LM()
...
%
%This version uses the Levenberg-Marquardt (LM) algorithm
%
...
for IRepeat = 1:NRepeat
    ...
    while ~Converged && ~Aborted
        ...
        %Do the least squares solution using Lambda/Nu as the damping factor
        %Note:diag(Matrix) extracts the diagonal of a matrix as a vector
        %diag(Vector) creates a diagonal matrix with the specified elements
        %on the main diagonal
        Mat1 = BaseMat + Lambda/Nu * diag(diag(BaseMat));
        DeltaVec1 = Mat1\(AfMat.' * WMat * YfVec);
        ThetaVec1 = Theta0 + DeltaVec1;
        ResidVec1 = (lMyFun(XVec, ThetaVec1) - YMeas).';
        ...
        ResidVec2 = (lMyFun(XVec, ThetaVec2) - YMeas).';
